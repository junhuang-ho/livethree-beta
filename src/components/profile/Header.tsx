...
import {
    END_CALL_BUFFER_SECONDS_EXTRA, DELETE_PERMISSION,
    CHAIN_NUMBER_TO_CHAIN_NAME_MAPPING, CHAIN_NUMBER_POLYGON
...
export const Header = ({ uid, dataUser, showcaseMode, isUserLoading, reloadUser, inFavPage }: any) => {
    ...
    const [dataUserLocal] = useDocumentData<any>(doc(COL_REF_USERS, localAddress));
    const [dataRoomActive, loadingRoomActive, errorRoomActive] = useDocumentData(doc(getColRefActive(uid), localAddress));
    ...
    const [previewMode] = usePreviewMode(localAddress, uid)

    const [displayHasNetFlowModal, setDisplayHasNetFlowModal] = useState<boolean>(false)

    const [isFavourited, setIsFavourited] = useState(false)
    useEffect(() => {
        setIsFavourited(dataUserLocal?.favourites.includes(uid))
    }, [dataUserLocal])
    ...
    useUpdateEffect(() => {
        const cleanUpAfterCalleeRejectCall = async () => {
            await cleanUp(uid, localAddress)
                        ...
                        <Stack direction={ isMobile ? "column" : "row" } alignItems="center" spacing={ 3 }>
                            { editHeader ? (
                                ...
                                <Stack alignItems="center" justifyContent="center">
                                    <IconButton
                                        onClick={ async () => {
                                            const flowInfo = await getFlowData(localAddress, uid)
                            ...
                            <Stack justifyContent="space-evenly">
                                ...
                                { editHeader ? (
                                    ...
                                    <Stack
                                        ...
                                        alignItems="center"
                                    >
                                        <Typography variant="body1">Address { isMobile ? shortenAddress(uid) : uid }</Typography>
                                        <ButtonCopy value={ uid } />
                                    </Stack>
                                ) }
                                { editHeader ? (
                                    <Stack>
                            ...
                            { isMobile && !showcaseMode && uid === localAddress &&
                                ...
                                >
                                    <Typography>
                                        Connected to { CHAIN_NUMBER_TO_CHAIN_NAME_MAPPING[chainId] } network.
                                    </Typography>
                                    <Stack direction="row" alignItems="center">
                                        <Typography>
                                            Token balances
                        ...
                        <Stack justifyContent="center">
                            { isFavourited ? (
                                <Tooltip title="Unfollow">
                                    <StandardButton
                                        ...
                                        color="secondary"
                                        onClick={ async () => {
                                            await updateDoc(doc(COL_REF_USERS, localAddress), {
                            ...
                            <Box>
                                                    ...
                                                    if (previewMode) {
                                                        ...
                                                        if (chainId !== 0 && !isUserLoading && dataUser?.online && !dataUser?.isActive) {
                                                            ...
                                                            if (!hasDeletePermission) {
                                                                ...
                                                                const hasSufficientGas = await hasGasToCreateAndDeleteFlow(uid, dataUser.flowRate)
                                ...
                                <DialogPreCallDetails
                                    ...
                                    deposit={ deposit }
                                    restartTimer={ restartTimer } address={ uid } flowRate={ dataUser.flowRate }

...
import { StandardButton, StyledGasBadge } from "../styled";
...
import { GLOBAL_MAX_CHARACTER_NUMBER, CHAIN_NUMBER_POLYGON } from "../../constants/constants";
            ...
            <CardContent>
                <Stack spacing={ 1 }>
                    <Typography align="justify">
                        ...
                        { ` ${ tokenXSymbol ? tokenXSymbol : "" }` } is the token streamed during your money streaming video call.
                    </Typography>
                            ...
                            { tokenAllowance === "0" ? (
                                <Stack spacing={ 1 }>
                                    ...
                                    <TextField
                                        ...
                                        error={ (!!wrapAmount && Number(wrapAmount) === 0) || (chainId === CHAIN_NUMBER_POLYGON ? (!!tokenBalance && !!wrapAmount && ethers.BigNumber.from(tokenBalance).lt(ethers.utils.parseUnits(wrapAmount, 6))) : (!!tokenBalance && !!wrapAmount && ethers.BigNumber.from(tokenBalance).lt(ethers.utils.parseEther(wrapAmount)))) }
                                        helperText={ (!!wrapAmount && Number(wrapAmount) === 0) && ERROR_ZERO_AMOUNT || (chainId === CHAIN_NUMBER_POLYGON ? (!!tokenBalance && !!wrapAmount && ethers.BigNumber.from(tokenBalance).lt(ethers.utils.parseUnits(wrapAmount, 6))) : (!!tokenBalance && !!wrapAmount && ethers.BigNumber.from(tokenBalance).lt(ethers.utils.parseEther(wrapAmount)))) && INSUFFICIENT_BALANCE }
                                    // disabled={  }
                                    />
                                </Stack>
                            ) : (
                                <Stack spacing={ 1 }>
                                    <Box sx={ { backgroundColor: "warning.main" } }>
                                        <Typography align="justify">
                                            ...
                                            - You currently have { tokenAllowance ? (chainId === CHAIN_NUMBER_POLYGON ? ethers.utils.formatUnits(tokenAllowance, 6) : ethers.utils.formatEther(tokenAllowance)) : "" } amount of { tokenSymbol ? tokenSymbol : "" } approved
                                ...
                                disabled={
                                    ...
                                    (!toggled ?
                                        (tokenAllowance === "0" ? (
                                            !wrapAmount
                                            || Number(wrapAmount) === 0
                                            || (chainId === CHAIN_NUMBER_POLYGON ? (tokenBalance && wrapAmount && ethers.BigNumber.from(tokenBalance).lt(ethers.utils.parseUnits(wrapAmount, 6))) : (tokenBalance && wrapAmount && ethers.BigNumber.from(tokenBalance).lt(ethers.utils.parseEther(wrapAmount))))
                                        ) : (
                                            false
                                        )) : (
                                            !unwrapAmount

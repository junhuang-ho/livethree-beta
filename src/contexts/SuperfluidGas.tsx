...
import {
    CHAIN_NUMBER_POLYGON, CHAIN_NUMBER_MUMBAI,
    BYTES, ZERO_BYTES, GAS_LIMIT_MULTIPLIER_NUMERATOR,
    GAS_LIMIT_MULTIPLIER_DENOMINATOR,
...
export const SuperfluidGasProvider = ({ children }: { children: JSX.Element }) => {
    const { address: localAddress, web3authProvider: web3authUser, chainId, signer, rpcProvider: provider, refreshNativeBalance, getBalance } = useWeb3Auth()
    ...
    useEffect(() => {
        ...
        if (chainId && chainId === CHAIN_NUMBER_POLYGON) {
        ...
        } else if (chainId && chainId === CHAIN_NUMBER_MUMBAI) {
